...
def calculate_metrics_rmu(model,
    ...
    for dataset_name in dataset_names:
        ...
        if dataset_name in metric_params:
            ...
            metric_param = {'target_metric': 'correct'}
    ...
    # if compute_loss and activation_store is not None:
...
def calculate_metrics_side_effects(
    model,
    sae,
    main_ablate_params,
    sweep,
    ...
    for ablate_params in tqdm(ablate_params_list):
        ...
        for dataset_name in dataset_names:
            ...
            # print(dataset_metrics['total_correct'], baseline_metrics['total_correct'])
            # print(dataset_metrics['mean_correct'], baseline_metrics['mean_correct'])

            if dataset_metrics['total_correct'] >= baseline_metrics['total_correct'] - thresh:
                metrics_for_current_ablation[dataset_name] = dataset_metrics
            else:
                break
        
        
        if len(metrics_for_current_ablation.keys()) == len(dataset_names):
            metrics_for_current_ablation['ablate_params'] = ablate_params
            metrics_list.append(metrics_for_current_ablation)
