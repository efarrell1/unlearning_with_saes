    ...
    for epoch in range(1):
        ...
        with tqdm.tqdm(total=num_batches) as pbar:
            for idx in range(num_batches):
                ...
                attention_mask = unlearn_inputs.attention_mask.unsqueeze(dim=-1)
                updated_forget_activations = forward_with_cache(
                    updated_model, unlearn_inputs, module=updated_module, no_grad=False
                ).to(updated_model.device)
                
                masked_updated_forget_activations = torch.where(
                    attention_mask == 1,
                    updated_forget_activations,
                    torch.zeros_like(updated_forget_activations)
                )
                masked_control_vec = torch.where(
                    attention_mask == 1,
                    control_vec,
                    torch.zeros_like(control_vec)
                )
                unlearn_loss = torch.nn.functional.mse_loss(
                    masked_updated_forget_activations, masked_control_vec, reduction='sum'
                ) / attention_mask.sum()
                ...
                attention_mask = retain_inputs.attention_mask.unsqueeze(dim=-1)
                ...
                masked_updated_retain_activations = torch.where(
                    attention_mask == 1,
                    updated_retain_activations,
                    torch.zeros_like(updated_retain_activations)
                )
                masked_frozen_retain_activations = torch.where(
                    attention_mask == 1,
                    frozen_retain_activations,
                    torch.zeros_like(frozen_retain_activations)
                )

                # Compute the MSE loss only on the masked activations
                retain_loss = torch.nn.functional.mse_loss(
                    masked_updated_retain_activations, masked_frozen_retain_activations, reduction='sum'
                ) / attention_mask.sum()
...
def get_args():
    ...
    ### Model arguments
    parser.add_argument(
        "--model_name_or_path", type=str, default="google/gemma-2-2b-it"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--forget_corpora",
        type=str,
        default="bio-forget-corpus,bio-forget-corpus",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="placeholder for weird py notebook bug",
    )
    ### rmu hyperparameters
